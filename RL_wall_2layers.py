...
class DQN (torch.nn.Module):
    ...
    def __init__ ( self:              Self,
        ...
        self.linear1 = torch.nn.Linear(torch.prod(torch.tensor(data_shape)), 128)
        self.linear4 = torch.nn.Linear(128, actions_dimension)
...
def compute_reward ( state:                torch.tensor,
    ...
    # if the rat is near the walls, we give a negative reward
    if next_to_wall(player_locations):
        # print(player_locations["RL"])
        # print(next_to_wall(player_locations))
        reward -= 2
...
def next_to_wall(player_locations):
    if player_locations["RL"] < MAZE_WIDTH:
        return True
    if player_locations["RL"] > MAZE_WIDTH*(MAZE_HEIGHT-1)-1:
        return True
    if (player_locations["RL"] % MAZE_WIDTH) == 0:
        return True
    if (player_locations["RL"] % MAZE_WIDTH) == MAZE_WIDTH-1:
        return True
    return False
